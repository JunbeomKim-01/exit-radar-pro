...
function computeFeatures(bars: DailyIndicatorRow[]): ReversalFeatures | null {
  ...
  const vol20dma = sma(volValues, 20);
  ...
  const core: SignalBreakdown[] = [
    {
      name: "VXN 재상승",
      maxScore: 20,
      score: f.vxnChange3d > 5 ? 20 : f.vxnChange3d > 2 ? 12 : f.vxnChange3d > 0 ? 6 : 0,
  ...
  const support: SignalBreakdown[] = [
    {
      name: "VIX 급등",
      maxScore: 10,
      score: f.vixChange3d > 10 ? 10 : f.vixChange3d > 5 ? 6 : f.vixChange3d > 0 ? 2 : 0,
    ...
    },
    {
      name: "DXY 강세",
      maxScore: 5,
      score: f.dxyChange5d > 1 ? 5 : f.dxyChange5d > 0.3 ? 3 : 0,
    ...
    },
    {
      name: "WTI 약세",
      maxScore: 5,
      score: f.wtiChange5d < -3 ? 5 : f.wtiChange5d < -1 ? 3 : 0,
    ...
    },
    {
      name: "거래량 경고",
      maxScore: 5,
      score: f.volumeVs20dma > 1.5 ? 5 : f.volumeVs20dma > 1.2 ? 3 : 0,
  ...
  const core: SignalBreakdown[] = [
    {
      name: "VXN 둔화",
      maxScore: 20,
      score: f.vxnChange3d < -5 ? 20 : f.vxnChange3d < -2 ? 14 : f.vxnChange3d < 0 ? 6 : 0,
  ...
  const support: SignalBreakdown[] = [
    {
      name: "VIX 둔화",
      maxScore: 10,
      score: f.vixChange3d < -5 ? 10 : f.vixChange3d < -2 ? 6 : f.vixChange3d < 0 ? 2 : 0,
      description: `VIX 3일 변화 ${f.vixChange3d.toFixed(1)}%`,
    ...
    },
    {
      name: "WTI 약세 종료",
      maxScore: 5,
      score: f.wtiChange5d > 2 ? 5 : f.wtiChange5d > 0 ? 3 : 0,
      description: `원유 5일 변화 ${f.wtiChange5d.toFixed(1)}%`,
    ...
    },
    {
      name: "거래량 확대",
      maxScore: 5,
      score: f.volumeVs20dma > 1.3 ? 5 : f.volumeVs20dma > 1.1 ? 3 : 0,
      description: `거래량/20DMA ${f.volumeVs20dma.toFixed(2)}x`,
...
export async function analyzeReversal(): Promise<ReversalResult> {
  ...
  if (bars.length < 25) {
    throw new Error("시장 데이터가 충분하지 않습니다 (최소 25일 필요)");
  }
